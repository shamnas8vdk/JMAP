  ...
  clazz = declare(null, {
    ...
    ********************************************************/
    ...
    _proesssWebTierAndSignin: function(appConfig){
      ...
      var portalUrl = appConfig.portalUrl;
	  //debugger;
      tokenUtils.manualSignIn(portalUrl).then(lang.hitch(this, function(webTier){
		  //debugger;
        isWebTier = webTier;
        var portal = portalUtils.getPortal(portalUrl);
    ...
    _processSignIn: function(portalUrl, isWebTier){
      ...
      if(window.appInfo.isRunInPortal){
		...
		this.rawAppConfig.appId = "0KOw35X8DJfGkwL8";
        if (this.portalSelf.supportsOAuth && this.rawAppConfig && this.rawAppConfig.appId && !isWebTier) {
